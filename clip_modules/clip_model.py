...
def build_model(state_dict: dict, context_length: int):
...
def load_clip(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit=True, context_length=77, download_root = None):
    """Load a CLIP model
    Parameters
    ----------
    name : str
    ...
    if not jit:
        model = build_model(state_dict or model.state_dict(), context_length).to(device)
        if str(device) == "cpu":
            model.float()
        return model
