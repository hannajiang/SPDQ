...
class DisFormer(nn.Module):
    ...
    def forward(self, batch_patch, att_query, obj_query,att_txt,obj_txt,com_txt):
        ...
        att_query = self.norm(att_query + self.sa(att_query,att_query,att_query))
        obj_query = self.norm(obj_query + self.sa(obj_query,obj_query,obj_query))
        
        att_feat = self.norm_att(self.att_ca(att_query,batch_patch))
        obj_feat = self.norm_obj(self.obj_ca(obj_query,batch_patch))
        com_feat = self.norm_com(self.com_ca(com_query,batch_patch))
...
class Troika_MQF(nn.Module):    
    ...
    def __init__(self, config, attributes, classes, offset):
        ...
        self.DisFormer = DisFormer(dim, dim//64, self.cross_attn_dropout)
        ...
        # )
        self.additional_visual_params_mhsa = self.add_visual_tunable_params2()
    ...
    def add_visual_tunable_params2(self):
        # adapter_num = 2 * self.clip.visual.transformer.layers
        adapter_num = self.clip.visual.transformer.layers
    ...
    def encode_image_with_adapter(self, x: torch.Tensor):
        ...
        for i_block in range(self.clip.visual.transformer.layers):
            ...
            # else:
            #     adapter_prompt = self.additional_prompt_generator[i_block](x.type(torch.float))
            #     x = torch.cat([x[0:1, :, :], adapter_prompt.type(self.dtype), x[2:, :, :]], dim=0)
    ...
    def forward(self, batch, idx, training=False, pairs=None):
        ...
        for i_element in range(self.token_ids.shape[0]):
            ...
            if i_element == 0:
                ...
                att_txt = text_batch[0:1,1:4,:].repeat(b,1,1)
            else:
                obj_text_feat = _text_features
                obj_txt = text_batch[0:1,1:4,:].repeat(b,1,1)
        ...
        # batch_img_features = [batch_img, self.attr_disentangler(batch_img.type(torch.float)).type(self.dtype), self.obj_disentangler(batch_img.type(torch.float)).type(self.dtype)]
        # com_feat = self.com_former(self.com_query.repeat(b,1,1),batch_patch)
        # att_feat = self.att_former(self.att_query.repeat(b,1,1),batch_patch)
        # obj_feat = self.obj_former(self.obj_query.repeat(b,1,1),batch_patch)
        ...
        for i_element in range(self.token_ids.shape[0]):
            ...
            # if torch.isnan(torch.matmul(normalized_img_features[i_element],self.clip.logit_scale.exp()*idx_text_features.T)).any() or torch.isinf(torch.matmul(normalized_img_features[i_element],self.clip.logit_scale.exp()*idx_text_features.T)).any():
            #     pdb.set_trace()
        
        # print(att_query.squeeze())
        # print(obj_query.squeeze())
        # loss_kd_att = F.kl_div(F.log_softmax(att_query.squeeze()/2,dim=1), F.softmax(att_text_feat[labels[1]]/2,dim=1), reduction='batchmean')
        # loss_kd_obj = F.kl_div(F.log_softmax(obj_query.squeeze()/2,dim=1), F.softmax(obj_text_feat[labels[2]]/2,dim=1), reduction='batchmean')
        ...
        # loss_kd_ = 0.01*F.mse_loss(com_feat_/ (1e-5+com_feat_.norm(dim=-1, keepdim=True)), normalized_img_features[0],reduction="mean")

        
        # print(com_feat,att_feat,obj_feat)    
        ...
        if training and pairs is not None:
            ...
            return logits, loss_image+loss_pair+loss_att+loss_obj+loss_kd
